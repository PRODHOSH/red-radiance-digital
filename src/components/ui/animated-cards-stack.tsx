...
>(
  (
    ...
    // Always call useMotionTemplate unconditionally (hooks rule), then pick based on variant
    const dropShadow = useMotionTemplate`drop-shadow(${dx}px ${dy}px ${blur}px rgba(0,0,0,${alpha}))`
    const filter = variant === "light" ? dropShadow : "none"
    ...
    return (
      ...
      >
        <div className="flex items-center">
          {[...Array(filledStars)].map((_, k) => (
            <svg key={`filled-${k}`} className="size-4 text-inherit" fill="currentColor" viewBox="0 0 20 20">
          ...
          {fractionalPart > 0 && (
            <svg className="size-4 text-inherit" fill="currentColor" viewBox="0 0 20 20">
              <defs>
                <linearGradient id="rr-star-half">
                  <stop offset={`${fractionalPart * 100}%`} stopColor="currentColor" />
                  <stop offset={`${fractionalPart * 100}%`} stopColor="rgb(55 65 81)" />
                </linearGradient>
              </defs>
              <path
                ...
                fill="url(#rr-star-half)"
              />
            </svg>
          )}
          {[...Array(emptyStars)].map((_, k) => (
            <svg key={`empty-${k}`} className="size-4 text-white/15" fill="currentColor" viewBox="0 0 20 20">
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.957a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.37 2.448a1 1 0 00-.364 1.118l1.287 3.957c.3.921-.755 1.688-1.54 1.118l-3.37-2.448a1 1 0 00-1.175 0l-3.37 2.448c-.784.57-1.838-.197-1.54-1.118l1.287-3.957a1 1 0 00-.364-1.118L2.05 9.384c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.95-.69l1.286-3.957z" />
            </svg>
          ))}
        </div>
